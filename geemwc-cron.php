...
function GEEMWC_cron_job_worker ($hardcron=false)
{
  ...
  $funds_received_value_expires_in_secs = $geemwc_settings['funds_received_value_expires_in_mins'] * 60;
  $assigned_address_expires_in_secs     = $geemwc_settings['assigned_address_expires_in_mins'] * 60;
  $confirmations_required = $geemwc_settings['confs_num'];
  ...
  $query =
    "SELECT * FROM `$geem_payments_table_name`
      WHERE
      (
  ...
  if (is_array($rows_for_balance_check))
  {
  	...
  	foreach ($rows_for_balance_check as $row_for_balance_check)
  	{
		  ...
		  $address_meta = GEEMWC_unserialize_address_meta(@$row_for_balance_check['address_meta']);
		  ...
		  // Retrieve current balance at address considering required confirmations number and api_timemout value.
			$address_request_array['geem_payment_id'] = $row_for_balance_check['geem_payment_id'];
      $address_request_array['geem_address'] = $row_for_balance_check['geem_address'];
      $address_request_array['block_index'] = $row_for_balance_check['index_in_wallet'];
		  ...
		  $balance_info_array = GEEMWC__getreceivedbyaddress_info($address_request_array, $geemwc_settings);
        ...
        $current_time = time();
        $query =
          "UPDATE `$geem_payments_table_name`
             SET
        ...
        if ($balance_info_array['balance'] > 0)
        {
          ...
          GEEMWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Detected non-zero balance at address: '{$row_for_balance_check['geem_address']}, Payment ID = {$row_for_balance_check['geem_payment_id']}, order ID = '{$last_order_info['order_id']}'. Detected balance ='{$balance_info_array['balance']}'.");

          if ($balance_info_array['balance'] < $last_order_info['order_total'])
          {
            GEEMWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: balance at address: '{$row_for_balance_check['geem_address']}, Payment ID = {$row_for_balance_check['geem_payment_id']}' (GEEM '{$balance_info_array['balance']}') is not yet sufficient to complete it's order (order ID = '{$last_order_info['order_id']}'). Total required: '{$last_order_info['order_total']}'. Will wait for more funds to arrive...");
          }
        }
		         ...
		         array (
		            'orders' =>
		               array (
		                  ...
		                  array (
		                     'order_id'     => $order_id,
		                     'order_total'  => $order_total_in_geem,
	        ...
	        GEEMWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Full payment for order ID '{$last_order_info['order_id']}' detected at address: '{$row_for_balance_check['geem_address']}, Payment ID = {$row_for_balance_check['geem_payment_id']}' (GEEM '{$balance_info_array['balance']}'). Total was required for this order: '{$last_order_info['order_total']}'. Processing order ...");
	        ...
	        GEEMWC__process_payment_completed_for_order ($last_order_info['order_id'], $balance_info_array['balance']);
          ...
          //
	        $query =
	          "UPDATE `$geem_payments_table_name`
	            ...
	            WHERE `id`='$row_id';";
	        $ret_code = $wpdb->query ($query);
	        GEEMWC__log_event (__FILE__, __LINE__, "Cron job: SUCCESS: Order ID '{$last_order_info['order_id']}' successfully completed.");

		    }
		  }
		  else
		  {
		    GEEMWC__log_event (__FILE__, __LINE__, "Cron job: Warning: Cannot retrieve balance for address: '{$row_for_balance_check['geem_address']} Payment ID: {$row_for_balance_check['geem_payment_id']}: " . $balance_info_array['message']);
		  }
		  //..//
		}
	}
  ...
  //         GEEMWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Detected LATE PAYMENT non-zero balance at address: '{$row_for_balance_check['geem_address']}, Payment ID = {$row_for_balance_check['geem_payment_id']}, order ID = '{$last_order_info['order_id']}'. Detected balance ='{$balance_info_array['balance']}'.");
  ...
  //           // Apply exchange rate multiplier only for stores with non-Geem default currency.
  //           $order_total_in_geem = $order_total_in_geem;

  //         $order_total_in_geem   = sprintf ("%.8f", $order_total_in_geem);

  //         $difference = $order_total_in_geem - $last_order_info['order_total'];
  ...
  //                                 __('New Order Total: ', 'woocommerce') . $order_total_in_geem . ' '.
  //                                 __('Difference: ', 'woocommerce') . $order_total_in_geem . ' (' . $diffincur . ' '. get_woocommerce_currency() .')'
  //                               );

  //         if ($balance_info_array['balance'] < $last_order_info['order_total'])
  //         {
  //           GEEMWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: LATE PAYMENT balance at address: '{$row_for_balance_check['geem_address']}, Payment ID = {$row_for_balance_check['geem_payment_id']}' (GEEM '{$balance_info_array['balance']}') is not yet sufficient to complete it's order (order ID = '{$last_order_info['order_id']}'). Total required: '{$last_order_info['order_total']}'. Will wait for more funds to arrive...");
  //         }
  //       }
  ...
  //         // Last order was fully paid! Complete it...
  //         GEEMWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Full LATE PAYMENT for order ID '{$last_order_info['order_id']}' detected at address: '{$row_for_balance_check['geem_address']}, Payment ID = {$row_for_balance_check['geem_payment_id']}' (GEEM '{$balance_info_array['balance']}'). Total was required for this order: '{$last_order_info['order_total']}'. Processing order ...");
  ...
  //             WHERE `id`='$row_id';";
  //         $ret_code = $wpdb->query ($query);
  //         GEEMWC__log_event (__FILE__, __LINE__, "Cron job: SUCCESS LATE PAYMENT: Order ID '{$last_order_info['order_id']}' successfully completed.");
  //       }
  //     }
  //     else
  //     {
