...
//===========================================================================
function GEEMWC__generate_new_Geem_payment_id ($geemwc_settings=false, $order_info)
{
  global $wpdb;
  ...
  $new_geem_payment_id = $wallet_api->makePaymentId();
  ...
  $address_request_array['geem_address'] = $geem_address;
  $address_request_array['geem_payment_id'] = $new_geem_payment_id;
  ...
  $address_request_array['api_timeout'] = $geemwc_settings['blockchain_api_timeout_secs'];
  $ret_info_array = GEEMWC__getreceivedbyaddress_info ($address_request_array, $geemwc_settings);
  ...
  $address_meta_serialized = GEEMWC_serialize_address_meta ($address_meta);
  //$remote_addr  = $order_info['requested_by_ip'];

  // Insert newly generated address into DB
  $query = "INSERT INTO `$geem_payments_table_name` (`geem_address`, `geem_payment_id`, `origin_id`, `index_in_wallet`, `status`, `total_received_funds`, `received_funds_checked_at`, `assigned_at`, `address_meta`) VALUES ('$geem_address', '$new_geem_payment_id', 'none', '$next_key_index', '$status', '$funds_received', '$received_funds_checked_at_time', '$assigned_at_time', '$address_meta_serialized');";
  $ret_code = $wpdb->query($query);

  return $new_geem_payment_id;
}
//===========================================================================

//===========================================================================
// Function makes sure that returned value is valid array
function GEEMWC_unserialize_address_meta ($flat_address_meta)
{
...
function GEEMWC_serialize_address_meta ($address_meta_arr)
{
   return GEEMWC__safe_string_escape(serialize($address_meta_arr));
}
//===========================================================================
...
function GEEMWC__getreceivedbyaddress_info ($address_request_array, $geemwc_settings=false)
{
	if (!$geemwc_settings)
  	$geemwc_settings = GEEMWC__get_settings ();

  $geem_address            = $address_request_array['geem_address'];
	$geem_payment_id         = $address_request_array['geem_payment_id'];
  ...
  $t = $fnw->getTransactions( $status["blockCount"] - 50000, false, 50000, $geem_payment_id, [$geem_address]);
  // print_r( $t );
...
//===========================================================================
// Returns:
//    success: number of currency units (dollars, etc...) would take to convert to 1 Geem, ex: "15.32476".
//    failure: false
//
...
function GEEMWC__get_exchange_rate_per_Geem ($currency_code, $rate_retrieval_method = 'getfirst', $get_ticker_string=false)
{
   if ($currency_code == 'GEEM')
      return "1.00";   // 1:1

	$geemwc_settings = GEEMWC__get_settings ();
  $exchange_rate_type = $geemwc_settings['exchange_rate_type'];
  $exchange_multiplier = $geemwc_settings['exchange_multiplier'];
  if (!$exchange_multiplier)
	...
	$ticker_string = "<span style='color:#222;'>According to your settings (including multiplier), current calculated rate for 1 Geem (in {$currency_code})={{{EXCHANGE_RATE}}}</span>";
	...
	$this_currency_info = @$geemwc_settings['exchange_rates'][$currency_code][$requested_cache_method_type];
	if ($this_currency_info && isset($this_currency_info['time-last-checked']))
	{
	  $delta = $current_time - $this_currency_info['time-last-checked'];
	  if ($delta < (@$geemwc_settings['cache_exchange_rates_for_minutes'] * 1))
	  {
  ...
  $exchange_rate = GEEMWC__get_exchange_rate_from_cryptocompare($currency_code, $exchange_rate_type, $geemwc_settings); 

  // Save new currency exchange rate info in cache
  GEEMWC__update_exchange_rate_cache ($currency_code, $requested_cache_method_type, $exchange_rate);
...
//===========================================================================
function GEEMWC__function_not_exists ($fname) { return !function_exists($fname); }
//===========================================================================

//===========================================================================
function GEEMWC__update_exchange_rate_cache ($currency_code, $requested_cache_method_type, $exchange_rate)
{
  // Save new currency exchange rate info in cache
  $geemwc_settings = GEEMWC__get_settings ();   // Re-get settings in case other piece updated something while we were pulling exchange rate API's...
  $geemwc_settings['exchange_rates'][$currency_code][$requested_cache_method_type]['time-last-checked'] = time();
  $geemwc_settings['exchange_rates'][$currency_code][$requested_cache_method_type]['exchange_rate'] = $exchange_rate;
  GEEMWC__update_settings ($geemwc_settings);
...
function GEEMWC__get_exchange_rate_from_cryptocompare ($currency_code, $rate_type, $geemwc_settings)
{
 $source_url = "https://raw.githubusercontent.com/geemcoin/geem-woocommerce/master/rate.json"; // Manually get Geem exchange rate from GitHub instead of a 3rd Party data provider API. 
 $result = @GEEMWC__file_get_contents ($source_url, false, $geemwc_settings['exchange_rate_api_timeout_secs']);
...
*/
function GEEMWC__file_get_contents ($url, $return_content_on_error=false, $timeout=60, $user_agent=FALSE, $is_post=false, $post_data="")
{
		...
		foreach ($post_data as $k => $v)
			{
				...
				if (is_object($safetied))
					$safetied = GEEMWC__object_to_array($safetied);
				if (is_array($safetied))
				{
					$safetied = serialize($safetied);
					$safetied = $p . str_replace('=', '_', GEEMWC__base64_encode($safetied));
...
//===========================================================================
function GEEMWC__object_to_array ($object)
{
	if (!is_object($object) && !is_array($object))
    return $object;
  return array_map('GEEMWC__object_to_array', (array) $object);
}
//===========================================================================

//===========================================================================
// Credits: http://www.php.net/manual/en/function.mysql-real-escape-string.php#100854
function GEEMWC__safe_string_escape ($str="")
{
   $len=strlen($str);
   $escapeCount=0;
   $targetString='';
...
//===========================================================================
// Syntax:
//    GEEMWC__log_event (__FILE__, __LINE__, "Hi!");
//    GEEMWC__log_event (__FILE__, __LINE__, "Hi!", "/..");
//    GEEMWC__log_event (__FILE__, __LINE__, "Hi!", "", "another_log.php");
function GEEMWC__log_event ($filename, $linenum, $message, $prepend_path="", $log_file_name='__log.php')
{
   $log_filename   = dirname(__FILE__) . $prepend_path . '/' . $log_file_name;
   $logfile_header = "<?php exit(':-)'); ?>\n" . '/* =============== GeemWC LOG file =============== */' . "\r\n"; //OMG -- The Magic Log file!
   $logfile_tail   = "\r\nEND";
   ...
   if ($fhandle)
      {
      @fwrite ($fhandle, "\r\n// " . $_SERVER['REMOTE_ADDR'] . '(' . $_SERVER['REMOTE_PORT'] . ')' . ' -> ' . date("Y-m-d, G:i:s T") . "|" . GEEMWC_VERSION . "/" . GEEMWC_EDITION . "|$filename($linenum)|: " . $message . $logfile_tail);
      @fclose ($fhandle);
      }
}
//===========================================================================
...
//===========================================================================
function GEEMWC__send_email ($email_to, $email_from, $subject, $plain_body)
{
   $message = "
   <html>
   <head>
...
//===========================================================================
function GEEMWC__is_gateway_valid_for_use (&$ret_reason_message=NULL)
{
  $valid = true;
  $geemwc_settings = GEEMWC__get_settings ();
...
////   'service_provider'                     =>  'local_wallet',    // 'blockchain_info'
  ...
  //----------------------------------
  // Validate settings
  if ($geemwc_settings['service_provider']=='local_wallet')
  {         
          $geemwc_settings = GEEMWC__get_settings();
          $address = $geemwc_settings['address'];

          try{
            $wallet_api = New ForkNoteWalletd("http://127.0.0.1:8070");  // Change this to your server.
...
function GEEMWC__base64_decode($input)
{
...
function GEEMWC__base64_encode($data)
{
    ...
    $o1 = GEEMWC_charCodeAt($data, $i++);
    $o2 = GEEMWC_charCodeAt($data, $i++);
    $o3 = GEEMWC_charCodeAt($data, $i++);
    $bits = $o1 << 16 | $o2 << 8 | $o3;
    $h1 = $bits >> 18 & 0x3f;
    $h2 = $bits >> 12 & 0x3f;
    $h3 = $bits >> 6 & 0x3f;
    ...
    $tmp_arr[$ac++] = GEEMWC_charAt($b64, $h1).GEEMWC_charAt($b64, $h2).GEEMWC_charAt($b64, $h3).GEEMWC_charAt($b64, $h4);
    } while ($i < strlen($data));
    $enc = implode($tmp_arr, '');
    $r = (strlen($data) % 3);
...
function GEEMWC_charCodeAt($data, $char) {
    return ord(substr($data, $char, 1));
}

function GEEMWC_charAt($data, $char) {
